...
static uint32_t stamp_count_exit;
	...
	uint32_t cri_cycle_count[2];
	uint32_t cri_instr_count[2];
#endif
		...
		if (handler != NULL) {
...
#endif
#ifdef CFG_TIMER_SUPPORT
#ifndef CFG_IRQ_DEBUG_APTIME
			intc_irq_desc[id].last_enter = ts_ostimer_read_isr(1);
#else
			intc_irq_desc[id].last_enter = ipi_get_ts();
#endif
#endif
#ifdef CFG_IRQ_CYCCOUNT_RECORD
			cycle_count[0] = get_cycle_count();
			instr_count[0] = get_instr_count();
#endif
#endif
			stamp_count_enter++;
...
#ifdef CFG_IRQ_DEBUG_SUPPORT
#ifdef CFG_IRQ_CYCCOUNT_RECORD
			cycle_count[1] = get_cycle_count();
			instr_count[1] = get_instr_count();
#endif
#ifdef CFG_XGPT_SUPPORT
...
#ifdef CFG_TIMER_SUPPORT
#ifndef CFG_IRQ_DEBUG_APTIME
			intc_irq_desc[id].last_exit = ts_ostimer_read_isr(1);
#else
			intc_irq_desc[id].last_exit = ipi_get_ts();
#endif
#endif
			...
			if (intc_irq_desc[id].last_exit
				...
				if ((duration > max_irq_limit)
					...
					cycle_count[0],	cycle_count[1], instr_count[0], instr_count[1]);
#endif
...
void intc_wakeup_source_count(void)
{
	...
	for (id = 0; id < INTC_IRQ_NUM; id++) {
		word = INTC_WORD(id);
		reg = INTC_IRQ_STA(word);
		if (DRV_Reg32(reg) & (1U << (id - 32 * word)))
...
#ifdef CFG_IRQ_CYCCOUNT_RECORD
		cri_cycle_count[0] = get_cycle_count();
		cri_instr_count[0] = get_instr_count();
#endif
...
void __wrap_vTaskExitCritical(void)
{
	if ((xGetCriticalNesting() - 1) == 0 && max_cs_limit != 0) {
#ifdef CFG_XGPT_SUPPORT
#ifdef CFG_IRQ_CYCCOUNT_RECORD
		cri_cycle_count[1] = get_cycle_count();
		cri_instr_count[1] = get_instr_count();
#endif
		...
		if (end_cs_time > start_cs_time) {
			...
			if (cs_duration_time > max_cs_limit) {
				PRINTF_D("critical section duration %llu>%llu, max %llu\n",
					cs_duration_time, max_cs_limit, max_cs_duration_time);
#ifdef CFG_IRQ_CYCCOUNT_RECORD
				PRINTF_E("Cycle count:%x %x, instruction count:%x %x\n",
				cri_cycle_count[0], cri_cycle_count[1], cri_instr_count[0], cri_instr_count[1]);
#endif
...
uint64_t get_cs_duration_time(void)
{
	return cs_duration_time;
}

void clr_irq_desc_irq_count(void)
{
	int i = 0;
	for(i=0; i< INTC_IRQ_NUM; i++){
		intc_irq_desc[i].irq_count = 0;
	}
}
