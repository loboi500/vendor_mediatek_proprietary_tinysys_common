	...
	do { \
		if (is_in_isr()) { \
			...
			if (xSemaphoreTake(g_aud_dma_lock, portMAX_DELAY) != pdTRUE) { \
				...
				if (scp_buf == NULL || dram_addr == NULL || size == 0) { \
					pr_notice("scp_buf %p dram_addr %p size %u err!!", dram_addr, scp_buf, size); \
				} else { \
	...
	do { \
		if (is_in_isr()) { \
			...
			if (xSemaphoreTake(g_aud_dma_lock, portMAX_DELAY) != pdTRUE) { \
				...
				if (dram_addr == NULL || scp_buf == NULL || size == 0) { \
					pr_notice("dram_addr %p scp_buf %p size %u err!!", dram_addr, scp_buf, size); \
				} else { \
...
int init_audio_ipi_dma(struct ipi_msg_t *msg)
{
	...
	if (!msg ||
	    msg->msg_id != AUD_CTL_MSG_A2D_DMA_INIT ||
	    msg->source_layer != AUDIO_IPI_LAYER_FROM_KERNEL ||
	    msg->param1 == 0) {
	...
	if (g_dma != NULL) {
		pr_info("already init. dma %p, phy %p/%llx, sz 0x%x, checksum %u, offset %u",
			g_dma,
			g_dma->base_phy.addr,
	...
	p_buf = (uint64_t *)(void*)msg->payload;
	g_dma = (struct audio_ipi_dma_t *)(uint32_t)ap_to_scp(p_buf[0]);
	if (!g_dma)
		return -EFAULT;

	dvfs_enable_DRAM_resource(OPENDSP_MEM_ID);

	AUD_ASSERT(g_dma->size == (uint32_t)p_buf[1]);
	...
	uint32_t count)
{
	...
	g_dump_buf = aud_kal_pvPortMalloc(dump_size);
			       ...
			       uint32_t *write_idx)
{
	...
	CACHE_INVALIDATE(&region->read_idx, ADSP_CACHE_ALIGN_BYTES); // CM4: 32, HiFi3/RV: 128
	...
	*write_idx = region->write_idx;
	...
	CACHE_FLUSH(&region->write_idx, ADSP_CACHE_ALIGN_BYTES); // CM4: 32, HiFi3/RV: 128
			      ...
			      uint32_t read_idx)
{
	...
	CACHE_INVALIDATE(&region->write_idx, ADSP_CACHE_ALIGN_BYTES); // CM4: 32, HiFi3/RV: 128
	DUMP_REGION(ipi_dbg, "region", region, data_size);
	...
	CACHE_FLUSH(&region->read_idx, ADSP_CACHE_ALIGN_BYTES); // CM4: 32, HiFi3/RV: 128
