...
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
#include <audio_task_ble_phone_call_dl.h>
#include <audio_task_ble_phone_call_ul.h>
#endif
...
#include <audio_task_fast.h>
#include <audio_task_ktv.h>
#include <audio_task_capture_raw.h>
#include <audio_task_fm_adsp.h>
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
#include <audio_task_bleums.h>
#include <audio_task_bleul.h>
#endif
#ifdef CFG_A2DP_OFFLOAD_SUPPORT
#include <audio_task_a2dp.h>
#endif
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
#include <audio_task_leaudio.h>
#endif
#endif
...
#ifdef CFG_SPK_PROCESS_IN_SCP_SUPPORT
#include <audio_task_spk_process.h>
#endif
...
static void create_all_audio_task(void);
static void destroy_all_audio_task(void);
#endif
...
static void create_all_audio_task()
{
...
#ifdef CFG_MTK_AURISYS_PHONE_CALL_SUPPORT
#if CFG_ADSP_CORE_ID == CFG_PHONE_CALL_CORE_ID
	create_audio_task(TASK_SCENE_PHONE_CALL, task_phone_call_new);
#if (configCORE_NUMBER == 1) && defined(CFG_PHONE_CALL_CORE_SEPARATE)
	create_audio_task(TASK_SCENE_PHONE_CALL_SUB, task_phone_call_sub_new);
#endif
#else
	...
	create_audio_task(TASK_SCENE_BLECALLDL, task_phone_call_ble_dl_new);
#elif CFG_ADSP_CORE_ID == CFG_ADSP_B
	create_audio_task(TASK_SCENE_BLECALLUL, task_phone_call_ble_ul_new);
#endif
#endif
	...
	create_audio_task(TASK_SCENE_MUSIC, task_music_new);
	create_audio_task(TASK_SCENE_CALL_FINAL, task_call_final_new);
	create_audio_task(TASK_SCENE_FAST, task_fast_new);
	create_audio_task(TASK_SCENE_FM_ADSP, task_fm_adsp_new);
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
	create_audio_task(TASK_SCENE_BLEDL, task_bleums_new);
#endif
#ifdef CFG_A2DP_OFFLOAD_SUPPORT
	create_audio_task(TASK_SCENE_A2DP, task_a2dp_new);
#endif
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
	create_audio_task(TASK_SCENE_BLEENC, task_leaudio_new);
#endif
#endif
#if defined(VOIP_SEPARATE_CORE) && (CFG_ADSP_CORE_ID == CFG_ADSP_A)
	create_audio_task(TASK_SCENE_VOIP, task_voip_new);
#endif
#if (CFG_ADSP_CORE_ID == CFG_ADSP_B) || (configCORE_NUMBER == 1)
#ifndef VOIP_SEPARATE_CORE
	create_audio_task(TASK_SCENE_VOIP, task_voip_new);
#endif
	create_audio_task(TASK_SCENE_CAPTURE_UL1, task_capture_ul1_new);
	create_audio_task(TASK_SCENE_KTV, task_ktv_new);
	create_audio_task(TASK_SCENE_CAPTURE_RAW, task_capture_raw_new);
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
	create_audio_task(TASK_SCENE_BLEUL, task_ble_ul_new);
	create_audio_task(TASK_SCENE_BLEDEC, task_leaudio_new);
#endif
#endif
#endif

#if (defined(CFG_MTK_AUDIODSP_SUPPORT) && defined(CFG_MTK_AUDIO_FRAMEWORK_SUPPORT))
#if CFG_ADSP_CORE_ID == CFG_ADSP_A
	create_audio_task(TASK_SCENE_AUD_DAEMON_A, task_daemon_new);
#elif CFG_ADSP_CORE_ID == CFG_ADSP_B
	create_audio_task(TASK_SCENE_AUD_DAEMON_B, task_daemon_new);
#else
	create_audio_task(TASK_SCENE_AUD_DAEMON_A, task_daemon_new);
#endif
#endif
...
#ifdef CFG_SPK_PROCESS_IN_SCP_SUPPORT
       create_audio_task(TASK_SCENE_RV_SPK_PROCESS, task_spk_process_new);
#endif
...
#elif CFG_ADSP_CORE_ID == CFG_ADSP_B
	create_audio_task(TASK_SCENE_AUDIO_CONTROLLER_HIFI3_B, task_controller_new);
#else
	create_audio_task(TASK_SCENE_AUDIO_CONTROLLER_HIFI3_A, task_controller_new);
#endif

#elif defined(CFG_SCP_AUDIO_FW_SUPPORT)
	create_audio_task(TASK_SCENE_AUDIO_CONTROLLER_RV, task_controller_new);
#endif
}
...
static void destroy_all_audio_task()
{
...
#if (configCORE_NUMBER == 1) && defined(CFG_PHONE_CALL_CORE_SEPARATE)
	destroy_audio_task(TASK_SCENE_PHONE_CALL_SUB, task_phone_call_delete);
#endif
#else
#ifdef CFG_PHONE_CALL_CORE_SEPARATE
	destroy_audio_task(TASK_SCENE_PHONE_CALL_SUB, task_phone_call_delete);
...
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
#if CFG_ADSP_CORE_ID == CFG_ADSP_A
	destroy_audio_task(TASK_SCENE_BLECALLDL, task_phone_call_ble_dl_delete);
#elif CFG_ADSP_CORE_ID == CFG_ADSP_B
	destroy_audio_task(TASK_SCENE_BLECALLUL, task_phone_call_ble_ul_delete);
#endif
#endif
	...
	destroy_audio_task(TASK_SCENE_MUSIC, task_music_delete);
	destroy_audio_task(TASK_SCENE_CALL_FINAL, task_call_final_delete);
	destroy_audio_task(TASK_SCENE_FAST, task_fast_delete);
	destroy_audio_task(TASK_SCENE_KTV, task_ktv_delete);
	destroy_audio_task(TASK_SCENE_CAPTURE_RAW, task_capture_raw_delete);
	destroy_audio_task(TASK_SCENE_FM_ADSP, task_fm_adsp_delete);
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
	destroy_audio_task(TASK_SCENE_BLEDL, task_bleums_delete);
	destroy_audio_task(TASK_SCENE_BLEUL, task_ble_ul_delete);
#endif
#ifdef CFG_A2DP_OFFLOAD_SUPPORT
	destroy_audio_task(TASK_SCENE_A2DP, task_a2dp_delete);
#endif
#ifdef CFG_LEAUDIO_OFFLOAD_SUPPORT
	destroy_audio_task(TASK_SCENE_BLEENC, task_leaudio_delete);
	destroy_audio_task(TASK_SCENE_BLEDEC, task_leaudio_delete);
#endif
#endif

#ifdef CFG_SPK_PROCESS_IN_SCP_SUPPORT
       destroy_audio_task(TASK_SCENE_RV_SPK_PROCESS, task_spk_process_delete);
#endif
...
#elif CFG_ADSP_CORE_ID == CFG_ADSP_B
	destroy_audio_task(TASK_SCENE_AUDIO_CONTROLLER_HIFI3_B, task_controller_delete);
#else
	destroy_audio_task(TASK_SCENE_AUDIO_CONTROLLER_HIFI3_A, task_controller_delete);
#endif
#elif defined(CFG_SCP_AUDIO_FW_SUPPORT)
	destroy_audio_task(TASK_SCENE_AUDIO_CONTROLLER_RV, task_controller_delete);
#endif
}
#endif /* end of !CFG_DO_ENABLED */
...
int create_audio_task(const uint8_t task_scene, new_task_t new_task)
{
	...
	if (task->create_data_task_loop != NULL)
		task->create_data_task_loop(task);
...
#if defined(CFG_MTK_AUDIO_FRAMEWORK_SUPPORT) || defined(CFG_SCP_AUDIO_FW_SUPPORT)
