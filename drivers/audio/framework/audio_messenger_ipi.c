...
#include <adsp_ipi.h>
#else
#if defined(CFG_MTK_AUDIO_USE_SCP_IPI)
#include <scp_ipi.h>
#else
...
void audio_messenger_ipi_init(void)
{
...
#else
#if defined(CFG_MTK_AUDIO_USE_SCP_IPI)
	AUD_LOG_D("+%s() scp ipi, Heap free = %d", __func__, xPortGetFreeHeapSize());
	...
	for (scene = 0; scene < TASK_SCENE_SIZE; scene++)
	    NEW_ALOCK(g_task_dma_lock[scene]);
	AUD_LOG_D("+%s() scp audio ipi, Heap free = %d", __func__, xPortGetFreeHeapSize());
	retval = scp_audio_ipi_registration(IPI_AUDIO, audio_ipi_msg_dispatcher, "audio");
	scp_audio_ipi_wakeup_ap_registration(IPI_AUDIO);
#endif
#endif
	if (retval != DONE)
...
int audio_send_ipi_msg_ack_back(ipi_msg_t *p_ipi_msg)
{
	if (p_ipi_msg->ack_type == AUDIO_IPI_MSG_NEED_ACK) {
		...
		int ret = 0;
		AUD_LOG_V("%s(), magic: %d, task: %d, id: 0x%x, ack: %d, p1: 0x%x, p2: 0x%x ",
			  __func__,
			  p_ipi_msg->magic,
			  p_ipi_msg->task_scene,
			  p_ipi_msg->msg_id,
