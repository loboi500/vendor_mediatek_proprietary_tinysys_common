...
int audio_dsp_hw_ap_irq(int core_id)
{
	...
	if (s_audio_dsp_hw_ops->audio_dsp_hw_ap_irq_op == NULL)
		return -EOPS;

	ret = s_audio_dsp_hw_ops->audio_dsp_hw_ap_irq_op(core_id);
	return ret;
}
...
int audio_dsp_hw_update_irq_cnt(int handle, unsigned int irq_cnt)
{
	...
	if (s_audio_dsp_hw_hal_ops == NULL)
		return -EOPS;
	if (s_audio_dsp_hw_hal_ops->audio_dsp_hw_update_irq_cnt_op == NULL)
		return -EOPS;
	ret = s_audio_dsp_hw_hal_ops->audio_dsp_hw_update_irq_cnt_op(handle, irq_cnt);
	return ret;
}
