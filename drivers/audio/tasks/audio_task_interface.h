...
typedef struct AudioTask {
	...
	TaskHandle_t freertos_data_task;
	...
	uint8_t data_queue_idx;
	struct ipi_msg_t *data_msg_array;
	uint8_t data_num_queue_element;
	xQueueHandle data_msg_idx_queue;
	...
	/* create RTOS task */
	void (*create_data_task_loop)(struct AudioTask *this);
	void (*destroy_data_task_loop)(struct AudioTask *this);
