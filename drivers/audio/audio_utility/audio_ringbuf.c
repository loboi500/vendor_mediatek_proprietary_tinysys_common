...
#ifdef CFG_SCP_AUDIO_FW_SUPPORT
			    ...
			    unsigned int count)
{
	...
	if (RingBuf1->pRead <= RingBuf1->pWrite) {
		unsigned int w2e = end - RingBuf1->pWrite;
...
void RingBuf_update_writeptr(struct RingBuf *RingBuf1, int count)
{
	if (count <= 0)
		return;

	if (RingBuf1->pRead <= RingBuf1->pWrite) {
		int w2e = RingBuf1->pBufEnd - RingBuf1->pWrite;
...
void RingBuf_update_readptr(struct RingBuf *RingBuf1, int count)
{
	if (count <= 0)
		return;
	...
	if (RingBuf1->pRead <= RingBuf1->pWrite) {
		...
		int r2e = RingBuf1->pBufEnd - RingBuf1->pRead;
			  ...
			  struct ringbuf_bridge *buf_bridge)
{
	...
	if (task_ring_buf->pRead == task_ring_buf->pWrite &&
		...
		AUD_LOG_D("%s task_ring_buf full\n", __func__);
		return 0;
	}
	...
	} else if (writeidx < task_ring_buf->pWrite) {
		datacount = task_ring_buf->bufLen -
			    (task_ring_buf->pWrite - writeidx);
	} else { // update data count
		if ((int)buf_bridge->datacount > task_ring_buf->datacount)
			datacount = buf_bridge->datacount - task_ring_buf->datacount;
		else
			datacount = 0;
	}
...
void dump_rbuf_s(const char *appendingstring, struct RingBuf *ring_buffer)
{
	...
	AUD_LOG_D("%s %s Base[%p] End[%p] R[%p] w[%p] Len[%d] count[%d] readidx = %d writeidx = %d\n",
		  ...
		  ring_buffer->bufLen,
		  ring_buffer->datacount,
		  (ring_buffer->pRead - ring_buffer->pBufBase),
		  (ring_buffer->pWrite - ring_buffer->pBufBase));
}
...
void dump_audio_hwbuffer_s(const char *appendingstring, struct audio_hw_buffer *audio_hwbuf)
{
	if (audio_hwbuf == NULL)
		return;

	AUD_LOG_D(
		"%s() %s hw_buffer: %d audio_memiftype: %d irq_num: %d memory_type: %d\n",
		appendingstring, __func__, audio_hwbuf->hw_buffer,
...
void dump_audio_hwbuffer(struct audio_hw_buffer *audio_hwbuf)
{
	...
	AUD_LOG_D(
		"%s hw_buffer: %d memiftype: %d irq_num: %d memory_type: %d\n",
...
#ifdef CFG_MTK_AUDIODSP_SUPPORT
	if (hal_cache_is_cacheable((void *)src_addr))
		hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_FLUSH,
					   (void *)src_addr, len);
#endif
		ret = (int)(DMA_RESULT)scp_dma_transaction(dst_addr,
					  src_addr,
					  len,
					  (DMA_ID)dma_channel,
					  NO_RESERVED);
#ifdef CFG_MTK_AUDIODSP_SUPPORT
	if (hal_cache_is_cacheable((void *)dst_addr))
			       ...
			       struct RingBuf *pRingBuf)
{
	...
	if (phyaddr > 0x20000000)
		pRingBuf->pBufBase = (char *)(uint32_t)ap_to_scp(phyaddr);

	else
		pRingBuf->pBufBase = (char *)(uint32_t)get_sram_addr((unsigned int)phyaddr);
				  ...
				  unsigned int count)
{
	...
	if (RingBuf1->pRead <= RingBuf1->pWrite) {
		...
		if (w2e > 0 && count <= (unsigned int)w2e) {
			memcpy(RingBuf1->pWrite, buf, count);
...
void RingBuf_cache_copyFromLinear_dma(struct RingBuf *RingBuf1, const char *buf,
				  unsigned int count, uint8_t isdram, int mem_id)
{
	unsigned int spaceIHave;
	char *end = RingBuf1->pBufBase + RingBuf1->bufLen;
	...
	if (RingBuf1->pRead <= RingBuf1->pWrite) {
		...
		if (w2e > 0 && count <= (unsigned int)w2e) {
			...
			dma_memcpy(RingBuf1->pBufBase, buf + w2e, count - w2e, isdram, mem_id);
			if ((((unsigned int)RingBuf1->pBufBase) % CACHE_LINE_SIZE) ||
			    ((count - w2e) % CACHE_LINE_SIZE)) {
				ALOGW("%s RingBuf1->pBufBase = %p count - w2 %d line %d\n", __func__,
				      RingBuf1->pBufBase,  count - w2e, __LINE__);
