...
enum {
	TASK_DEBUG_ERROR     = 0x1 << 0,
	TASK_DEBUG_WARNING   = 0x1 << 1,
	TASK_DEBUG_INFO      = 0x1 << 2,
	TASK_DEBUG_NOTICE    = 0x1 << 3,
	TASK_DEBUG_VERBOSE   = 0x1 << 4,
	/* tracing for DL write*/
	TASK_DEBUG_TRACING_DL   = 0x1 << 14,
};

#define task_print_debug(LOGLEVEL, ...) \
	((LOGLEVEL >= TASK_DEBUG_VERBOSE) \
	 ? AUD_LOG_D(__VA_ARGS__) \
	 : (void) 0)

#define task_print_info(LOGLEVEL, ...) \
	((LOGLEVEL >= TASK_DEBUG_INFO) \
	 ? AUD_LOG_D(__VA_ARGS__) \
	 : (void) 0)

#define task_print_notice(LOGLEVEL, ...) \
	((LOGLEVEL >= TASK_DEBUG_NOTICE) \
	 ? AUD_LOG_D(__VA_ARGS__) \
	 : (void) 0)
