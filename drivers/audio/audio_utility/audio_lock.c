...
#define LOCK_TIMEOUT_WARN_MS (500) /* 500 ms */

#define MAX_WAIT_BLOCKED_BY_LOCK_MS (5) /* 5 ms */
	...
	const char *file, const char *func, const uint32_t line)
{
	...
	if (xSemaphoreTake(p_alock->trace_info.idx_lock, portMAX_DELAY) == pdFALSE){
		AUD_LOG_E("%s(): fail to get semaphore\n", __FUNCTION__);
		return;
	}
...
#endif
	if (p_alock->lock_count < 0) {
		AUD_LOG_W("%p: atomic_dec(%s), %s, %s(), %uL, lock conut = %d",
			  p_alock, alock_name, file, func, line, p_alock->lock_count);
#ifdef MTK_AUDIO_LOCK_ENABLE_TRACE
		alock_dump_trace_info(p_alock);
#endif
		AUD_WARNING("lock count error!!");
	}
	return 0;
}
	...
	const char *file, const char *func, const uint32_t line)
{
	...
	if (retval == 0 && (time_stop - time_start) < (ms * 1000000ULL)) {
		ALOCK_LOG("%p: -wait(%s, %u), %s, %s(), %uL",
			  p_alock, alock_name, ms, file, func, line);
	} else {
		time_diff_wait = time_wait - time_start;
		time_diff_lock = time_stop - time_wait;
		time_diff_all_ms = (time_stop - time_start) / 1000000;

		if (time_diff_all_ms > ms + MAX_WAIT_BLOCKED_BY_LOCK_MS) {
	...
	const char *file, const char *func, const uint32_t line)
{
	...
	xQueueSendToBack(p_alock->cond, &tmp, 0);
