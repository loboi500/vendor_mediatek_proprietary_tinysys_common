...
{
	uint32_t size_out = (uint32_t)((size_in * buf_times) + 0.5); //add 0.5 for rounding

	/* align */
	if ((size_out % size_per_frame) != 0)
		size_out = ((size_out / size_per_frame) + 2) * size_per_frame;
			...
			void                **handler)
{
	...
	dvfs_add_swip(FORMATTER_FEATURE_ID, SWIP_ID_PROCESSING_SHIFTER,
		      &dvfs_config_bcv);
#endif
...
int aud_fmt_conv_destroy(void *handler)
{
	...
	trans_dvfs_cfg_src(&conv_hdl->source, &conv_hdl->target, &dvfs_config_src);
	trans_dvfs_cfg_bcv(&conv_hdl->source, &conv_hdl->target, &dvfs_config_bcv);
	dvfs_delete_swip(FORMATTER_FEATURE_ID, SWIP_ID_PROCESSING_SRC,
			 &dvfs_config_src);
