...
typedef struct aurisys_lib_handler_t {
	...
	data_buf_t slb_buf;
	data_buf_t syc_buf_d2u;
	data_buf_t syc_buf_u2d;
	void *arsi_handler;
	...
	audio_buf_t
	*p_dl_ref_buf_iv; /* only pointer to iv in lib_config->p_ul_ref_bufs */
	...
	*p_ul_ref_buf_aec; /* only pointer to aec in lib_config->p_ul_ref_bufs */
	...
	uint32_t arsi_process_type; /* hifi3 process type */
	uint32_t lib_order; /* lib order in scene */
	uint32_t lib_id; /* lib id for lib name */
