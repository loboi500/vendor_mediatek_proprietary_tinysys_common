...
int init_aurisys_controller(void *cfg_buf, uint32_t cfg_size)
{
	...
	ret = trans_aurisys_config_dsp_to_hal(cfg_buf, &g_controller->aurisys_config);
	if (ret < 0) {
		AUD_LOG_E("%s(), trans_aurisys_config_dsp_to_hal error!! return", __FUNCTION__);
		g_controller->aurisys_on = false;
...
bool aurisys_set_parameter(struct aurisys_adb_command_t *adb_cmd)
{
	...
	if (!strcmp(adb_cmd->adb_cmd_key, AURISYS_LIB_ALL_KEY)) {
		if (adb_cmd->aurisys_scenario != scenario_all) {
			...
			HASH_ITER(hh, the_scene_lib_table->all_library_name_list, itor_library_name,
				  tmp_library_name) {
	...
	aurisys_adb_command_t *adb_cmd)
{
	...
	uint32_t scenario_all = AURISYS_SCENARIO_DSP_ALL;
	uint32_t scenario_size = AURISYS_SCENARIO_DSP_SIZE;

	if ((library_config == NULL) || (adb_cmd == NULL)) {
		AUD_LOG_E("%s, fail! library_config: %p,  adb_cmd: %p\n",
			  __FUNCTION__, library_config, adb_cmd);
		return -1;
	}
	...
	uint32_t *wb_data_size)
{
	...
	if (!strcmp(adb_cmd->adb_cmd_key, AURISYS_LIB_ALL_KEY)) {
		if (adb_cmd->aurisys_scenario != scenario_all) {
			...
			HASH_ITER(hh, aurisys_config->library_config_hh, library_config,
				  tmp_library_config) {
				if (library_config != NULL) {
					AUD_LOG_W("%s get lib all, only get lib %s!!",
						  adb_cmd->adb_cmd_key, library_config->name);
					break;
				}
			}
		}
	...
	if (component == NULL) {
		AUD_LOG_W("%s, scene %d cannot find component!!",
			  adb_cmd->adb_cmd_key, adb_cmd->aurisys_scenario);
		ret = -1;
		goto AURISYS_GET_PARAM_EXIT;
	}
