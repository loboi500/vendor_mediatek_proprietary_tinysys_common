...
{
	if (!new_library_config) {
		AUD_LOG_E("%s(), new_library_config is NULL!", __FUNCTION__);
		AUD_ASSERT(new_library_config != NULL);
		return;
	}
	if (!new_library_config->name) {
		AUD_LOG_E("%s(), new_library_config->name is NULL!", __FUNCTION__);
...
{
	if (!new_component) {
		AUD_LOG_E("%s(), new_component is NULL!", __FUNCTION__);
		AUD_ASSERT(new_component != NULL);
		return;
	}
	HASH_ADD_INT(
		*hash_head,
...
{
	if (!new_scene_lib_table) {
		AUD_LOG_E("%s(), new_scene_lib_table is NULL!", __FUNCTION__);
		AUD_ASSERT(new_scene_lib_table != NULL);
		return;
	}
	HASH_ADD_INT(
		*hash_head,
...
{
	if (!new_library_name) {
		AUD_LOG_E("%s(), new_library_name is NULL!", __FUNCTION__);
		AUD_ASSERT(new_library_name != NULL);
		return;
	}
	if (!new_library_name->name) {
		AUD_LOG_E("%s(), new_library_name->name is NULL!", __FUNCTION__);
...
static void dump_library_config(aurisys_library_config_t *library_config)
{
	...
	HASH_ITER(hh, library_config, itor_lib, tmp_lib) {
		...
		if (itor_lib->api &&
		    itor_lib->api->arsi_get_arsi_version != NULL) {
			...
			if (itor_lib->api->arsi_get_arsi_version(&arsi_version) == 0) {
				if (strncmp(arsi_version.p_string, MTK_ARSI_API_VERSION,
					    sizeof(MTK_ARSI_API_VERSION)))
					AUD_LOG_W("arsi_version: \"%s\" not match current version!",
						  arsi_version.p_string);
			} else
				AUD_LOG_W("arsi_version: unknown arsi_version!");
			AUDIO_FREE_POINTER(arsi_version.p_string);
		} else
			AUD_LOG_W("arsi_version: arsi_get_arsi_version not implement!");
	...
	if (uplink_library_name_list) {
		HASH_ITER(hh, uplink_library_name_list, itor_library_name, tmp_library_name) {
			...
			if (new_library_name != NULL) {
				...
				AUD_LOG_E("%s(), add ul lib name %s fail!",
	...
	if (downlink_library_name_list) {
		HASH_ITER(hh, downlink_library_name_list, itor_library_name, tmp_library_name) {
			...
			if (new_library_name != NULL) {
				...
				AUD_LOG_E("%s(), add dl lib name %s fail!",
					  __FUNCTION__, itor_library_name->name);
			}
			new_library_name = NULL;
		}
...
static int trans_scene_lib_table_dsp_to_hal(
	...
	aurisys_scene_lib_table_t **hal)
{
	...
	for (idx_scene = 0; idx_scene < AURISYS_SCENARIO_DSP_SIZE; idx_scene++) {
		...
		if (new_scene_lib_table == NULL) {
			AUD_LOG_E("%s(), add new scence_lib_table idx_scene[%d] fail!",
				  __FUNCTION__, idx_scene);
			return -EEXIST;
		}
		...
		for (idx_lib = 0; idx_lib < num_ul_lib; idx_lib++) {
			...
			AUDIO_ALLOC_STRUCT(aurisys_library_name_t, new_library_name);
			if (new_library_name == NULL) {
				AUD_LOG_E("%s(), add UL new_library_name idx_scene[%d] "
					  "idx_lib [%d] fail!",
					  __FUNCTION__, idx_scene, idx_lib);
				return -EEXIST;
			}
		...
		for (idx_lib = 0; idx_lib < num_dl_lib; idx_lib++) {
			...
			AUDIO_ALLOC_STRUCT(aurisys_library_name_t, new_library_name);
			if (new_library_name == NULL) {
				AUD_LOG_E("%s(), add DL new_library_name idx_scene[%d] "
					  "idx_lib [%d] fail!",
					  __FUNCTION__, idx_scene, idx_lib);
				return -EEXIST;
			}
...
static int trans_library_config_dsp_to_hal(
	...
	aurisys_library_config_t **hal)
{
	...
	for (idx_lib = 0; idx_lib < MAX_TOTAL_LIB_NUM; idx_lib++) {
		...
		if (new_library_config == NULL) {
			AUD_LOG_E("%s(), add new_library_config idx_lib[%d] fail!",
				  __FUNCTION__, idx_lib);
			return -EEXIST;
		}
		...
		new_library_config->lib_id = idx_lib + 1; // start from 1

		AUDIO_ALLOC_STRUCT(struct AurisysLibInterface, new_library_config->api);
		if (new_library_config->api == NULL) {
			AUD_LOG_E("%s(), add new_library_config->api [%d] fail!",
				   __FUNCTION__, idx_lib);
			return -EEXIST;
		}
		...
		for (idx_comp = 0; idx_comp < AURISYS_SCENARIO_DSP_SIZE; idx_comp++) {
			...
			if (new_component == NULL) {
				AUD_LOG_E("%s(), add new_component idx_lib[%d] idx_comp[%d] fail!",
					  __FUNCTION__, idx_lib, idx_comp);
				return -EEXIST;
			}
			...
			if (lib_config_dsp->p_ul_buf_in.num_channels != 0 &&
				...
				if (buf_in == NULL) {
					AUD_LOG_E("%s(), add ul buf_in fail!\n", __FUNCTION__);
					return -EEXIST;
				}
				...
				if (buf_out == NULL) {
					AUD_LOG_E("%s(), add ul buf_out fail!\n", __FUNCTION__);
					return -EEXIST;
				}
				...
				if (lib_config->num_ul_ref_buf_array > 0) {
					AUDIO_ALLOC_STRUCT_ARRAY(audio_buf_t, lib_config->num_ul_ref_buf_array,
								 buf_refs);
					if (buf_refs == NULL) {
						AUD_LOG_E("%s(), add ul buf_refs fail!\n", __FUNCTION__);
						return -EEXIST;
					}
			...
			if (lib_config_dsp->p_dl_buf_in.num_channels != 0 &&
				...
				AUDIO_ALLOC_STRUCT(audio_buf_t, buf_in);
				if (buf_in == NULL) {
					AUD_LOG_E("%s(), add dl buf_in fail!\n", __FUNCTION__);
					return -EEXIST;
				}
				...
				AUDIO_ALLOC_STRUCT(audio_buf_t, buf_out);
				if (buf_out == NULL) {
					AUD_LOG_E("%s(), add dl buf_out fail!\n", __FUNCTION__);
					return -EEXIST;
				}
				...
				if (lib_config->num_dl_ref_buf_array > 0) {
					AUDIO_ALLOC_STRUCT_ARRAY(audio_buf_t, lib_config->num_dl_ref_buf_array,
								 buf_refs);
					if (buf_refs == NULL) {
						AUD_LOG_E("%s(), add dl buf_refs fail!\n", __FUNCTION__);
						return -EEXIST;
					}
...
int trans_aurisys_config_dsp_to_hal(
	...
	struct aurisys_config_t **hal)
{
	...
	if (new_cfg == NULL) {
		AUD_LOG_E("%s(), add new_cfg fail!\n", __FUNCTION__);
		return -EEXIST;
	}

	/* copy scene_lib_table */
	ret = trans_scene_lib_table_dsp_to_hal(dsp->scene_lib_table,
	...
	/* copy library_config */
	ret = trans_library_config_dsp_to_hal(dsp->library_config,
