...
status_t audioloud_arsi_set_key_value_pair(
	...
	void               *p_handler);
static AudioCompFltType_t convert_flt_by_stream_type(audio_stream_type_t
						     stream_type, bool is_drc, audio_devices_t output_devices);
//-----------------------------------------------------------
				       ...
				       *p_arsi_task_config, uint32_t drc_enable)
{
	...
	if (output_devices & AUDIO_DEVICE_OUT_SPEAKER) {
		acf_filtertype = convert_flt_by_stream_type(p_arsi_task_config->stream_type,
							    false, output_devices);
		if (drc_enable)     // DRC enable
			filtertype = convert_flt_by_stream_type(p_arsi_task_config->stream_type, true,
								output_devices);
			   ...
			   audioloud_lib_handle_t *pUser)
{
	...
	case BLOUD_PAR_SET_UPDATE_PARAM_TO_SWIP: {
		if (pUser->mState == BLOUD_STATE_OPEN) {
			...
			if (ret != 0) {
				if (print_log)
					print_log("%s mBloudHandle.Open failed", __FUNCTION__);
			}
		} else if (pUser->mState == BLOUD_STATE_PROCESS) {
			//check swip status
			BLOUD_HD_RuntimeStatus run_stat;
			memset(&run_stat, 0, sizeof(BLOUD_HD_RuntimeStatus));
			pUser->mBloudHandle.GetStatus(&pUser->mBloudHandle, &run_stat);
			if (run_stat.State == BLOUD_HD_SWITCHING_STATE) {
				if (print_log)
					print_log("%s dont set param when lib is in switch state", __FUNCTION__);
				break;
			}
			BLOUD_HD_RuntimeParam runtime_param;
			runtime_param.Command = BLOUD_HD_CHANGE_MODE;
			...
			if (ret != 0) {
				if (print_log)
					print_log("%s mBloudHandle.SetParameters failed ret = %d", __FUNCTION__, ret);
			}
		} else
			ret = -1;
		...
		if (ret != 0) {
			if (print_log)
				print_log("%s pUser->mState = %d", __FUNCTION__, pUser->mState);
		}
		if (print_log) {
...
AudioCompFltType_t convert_flt_by_stream_type(audio_stream_type_t stream_type,
		      ...
		      AUDIO_ACF_CUSTOM_PARAM_STRUCT *audioparam)   //ony use for v5
{
	...
	if (mFilterType == AUDIO_COMP_FLT_VIBSPK) {
		...
		if (mCheckStatus < 0) {
			ret = -1;
			if (print_log)
				print_log("pFilter_Coef_L mCheckStatus = %d, ", mCheckStatus);
		}
		mParamFormatUse.hpf_fc =
		...
		if (mCheckStatus < 0) {
			ret = -1;
			if (print_log)
				print_log("pFilter_Coef_R mCheckStatus = %d, ", mCheckStatus);
		}
	}
	...
	void                     *p_handler)
{
	...
	print_log("update_param mScenario = %d AudioMode = %d", lib_handler->mScenario,
		  AudioMode);
	// combine DRC + ACF param, check if param is the same to decide to do update
	if (p_arsi_task_config->output_device_info.devices & AUDIO_DEVICE_OUT_SPEAKER) {
		if (p_arsi_task_config->stream_type != AUDIO_STREAM_RING) {
			cmp_result = memcmp((void *)&lib_handler->mCachedAudioParam,
					    (void *)&lib_param->sDrcParam,
					    sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT));
			if (cmp_result != 0)
				memcpy((void *)&lib_handler->mCachedAudioParam, (void *)&lib_param->sDrcParam,
				       sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT));
	...
	void               *p_handler)
{
	...
	if (strstr(key_value_pair->p_string, "SetBesLoudnessStatus") != NULL &&
		...
		if (set_value == 0) {   //set process to bypass
			if (lib_handler->mBypassFlag & FILTER_BYPASS_ACF) {
				...
				lib_handler->mFilterType = convert_flt_by_stream_type(lib_handler->mStreamType,
	...
	if (strstr(key_value_pair->p_string, "SetAudioCustomScene") != NULL) {
		...
		if (lib_handler->mOutputDevice == AUDIO_DEVICE_OUT_SPEAKER) {
			...
			newAudioFilter = convert_flt_by_stream_type(set_value, pParamBuf->mDrcSwitch,
			...
			if (newAudioFilter == lib_handler->mFilterType)
				...
				switch (newAudioFilter) {
					print_log("old filter = %d new filter = %d", lib_handler->mFilterType,
						  newAudioFilter);
				case AUDIO_COMP_FLT_AUDIO:
				case AUDIO_COMP_FLT_DRC_FOR_MUSIC:
			...
			if (ret != 0)
				return BAD_VALUE;
			ret = set_lib_parameter(BLOUD_PAR_SET_UPDATE_PARAM_TO_SWIP, (void *)(long)0,
						lib_handler);
		} else
			return NO_ERROR;
	}
	...
	void         *p_handler)
{
	...
	if (strcmp(key_value_pair->p_string, "GetStatus") == 0) {
		...
		if (run_stat.State == BLOUD_HD_SWITCHING_STATE) {
			ret = snprintf(key_value_pair->p_string, key_value_pair->memory_size,
...
status_t audioloud_arsi_get_lib_version(string_buf_t *version_buf)
{
	...
	ret = snprintf(version_buf->p_string, version_buf->memory_size, "%d", Eng_Info.Version);
	if (ret < 0)
		return BAD_VALUE;
	return NO_ERROR;
}
