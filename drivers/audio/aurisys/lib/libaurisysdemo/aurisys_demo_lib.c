...
#define DEMO_LIB_VERSION "Ex: Version 1.5.0.0, Data: 2020-04-15"
...
static status_t demo_arsi_get_lib_version(string_buf_t *version_buf)
{
	if (version_buf == NULL)
		return BAD_VALUE;
...
static status_t demo_arsi_query_working_buf_size(
...
static status_t demo_arsi_create_handler(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
...
static status_t demo_arsi_query_sync_buf(
	data_buf_t *p_arsi_d2u_buf,
	data_buf_t *p_arsi_u2d_buf,
	void       *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;

	if (p_handler == NULL || p_arsi_d2u_buf == NULL || p_arsi_u2d_buf == NULL)
		return NO_INIT;

	lib_handler = (demo_lib_handle_t *)p_handler;

	if ((p_arsi_d2u_buf->memory_size < AURISYS_DEMO_SYNC_BUF_SIZE) ||
	    (p_arsi_u2d_buf->memory_size < AURISYS_DEMO_SYNC_BUF_SIZE)) {
		lib_handler->print_log("memory not enough, d2u: %d, u2d: %d!\n",
				       p_arsi_d2u_buf->memory_size, p_arsi_u2d_buf->memory_size);
		return NO_MEMORY;
	}

	/* setup u2d and d2u bufs */
	p_arsi_d2u_buf->data_size = AURISYS_DEMO_SYNC_BUF_SIZE;
	p_arsi_d2u_buf->p_buffer = (void *)lib_handler->d2u;

	p_arsi_u2d_buf->data_size = AURISYS_DEMO_SYNC_BUF_SIZE;
	p_arsi_u2d_buf->p_buffer = (void *)lib_handler->u2d;

	return NO_ERROR;
}

static status_t demo_arsi_query_max_debug_dump_buf_size(
...
static status_t demo_arsi_process_ul_buf(
	...
	void        *p_handler)
{
	...
	// setup u2d sync buf
	ret = snprintf(lib_handler->u2d, AURISYS_DEMO_SYNC_BUF_SIZE, "%s:%d",
		       "AURISYS_U2D", lib_handler->frame_index % 1000);
	if (ret < 0)
		lib_handler->print_log("U2D sync buf set error!\n");

	if (strncmp(lib_handler->d2u, "AURISYS_D2U", 11))
		lib_handler->print_log("D2U sync buf error!\n");
...
static status_t demo_arsi_process_dl_buf(
	...
	void        *p_handler)
{
	...
	// setup d2u sync buf
	ret = snprintf(lib_handler->d2u, AURISYS_DEMO_SYNC_BUF_SIZE, "%s:%d",
		       "AURISYS_D2U", lib_handler->frame_index % 1000);
	if (ret < 0)
		lib_handler->print_log("D2U sync buf set error!\n");

	if (strncmp(lib_handler->u2d, "AURISYS_U2D", 11))
		lib_handler->print_log("U2D sync buf error!\n");
...
static status_t demo_arsi_reset_handler(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	const data_buf_t         *p_param_buf,
	void                     *p_handler)
...
static status_t demo_arsi_destroy_handler(void *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;
...
static status_t demo_arsi_update_device(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	const data_buf_t         *p_param_buf,
	void                     *p_handler)
...
static status_t demo_arsi_update_param(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	const data_buf_t         *p_param_buf,
	void                     *p_handler)
...
static status_t demo_arsi_load_param(
	...
	const debug_log_fp_t      debug_log_fp)
{
	...
	if (g_private_buf == NULL) {
		...
		if (g_private_buf == NULL) {
			debug_log_fp("%s(), alloc private buf fail!\n", __func__);
			return NO_MEMORY;
		}
	}
...
static status_t demo_arsi_query_param_buf_size_by_custom_info(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	const string_buf_t       *product_info,
	const string_buf_t       *param_file_path,
...
static status_t demo_arsi_parsing_param_file_by_custom_info(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	const string_buf_t       *product_info,
	const string_buf_t       *param_file_path,
...
static status_t demo_arsi_set_addr_value(
	const uint32_t addr,
...
static status_t demo_arsi_get_addr_value(
	const uint32_t addr,
	uint32_t      *p_value,
	void          *p_handler)
{
...
static status_t demo_arsi_set_key_value_pair(
...
static status_t demo_arsi_get_key_value_pair(
...
static status_t demo_arsi_set_ul_digital_gain(
...
static status_t demo_arsi_set_dl_digital_gain(
...
static status_t demo_arsi_set_ul_mute(const uint8_t b_mute_on, void *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;
...
static status_t demo_arsi_set_dl_mute(const uint8_t b_mute_on, void *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;
...
static status_t demo_arsi_set_ul_enhance(const uint8_t b_enhance_on, void *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;
...
static status_t demo_arsi_set_dl_enhance(const uint8_t b_enhance_on, void *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;
...
static status_t demo_arsi_set_debug_log_fp(const debug_log_fp_t debug_log,
				    void *p_handler)
{
	demo_lib_handle_t *lib_handler = NULL;
...
static status_t demo_arsi_query_process_unit_bytes(
...
static status_t demo_arsi_set_buf(
	const uint32_t    id,
...
static status_t demo_arsi_get_buf(
	const uint32_t id,
...
static status_t demo_arsi_get_arsi_version(string_buf_t *version_buf)
{
	int ret = 0;
	if (version_buf == NULL)
		return BAD_VALUE;

	if (version_buf->memory_size < (strlen(MTK_ARSI_API_VERSION) + 1)) {
		version_buf->string_size = 0;
		return NOT_ENOUGH_DATA;
	}

	ret = snprintf(version_buf->p_string, version_buf->memory_size, "%s",
		       MTK_ARSI_API_VERSION);
	if (ret < 0)
		return BAD_VALUE;
	version_buf->string_size = strlen(MTK_ARSI_API_VERSION);

	return NO_ERROR;
}


static bool demo_arsi_is_support_slb()
{
	return true;
}


static status_t demo_arsi_query_working_buf_size_with_slb(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	data_buf_t               *p_arsi_slb_buf,
	uint32_t                 *p_working_buf_size,
	const debug_log_fp_t      debug_log_fp)
{
	if (p_arsi_task_config == NULL ||
	...
	/* slb */
	debug_log_fp("slb, size: %d, buff addr: %x\n",
		     p_arsi_slb_buf->memory_size, p_arsi_slb_buf->p_buffer);

	/* calculate working buffer working_buf_size by task config & lib config,
	   please use slb size and buffer to reduce working buffer size */
	*p_working_buf_size = sizeof(demo_lib_handle_t);
...
static status_t demo_arsi_create_handler_with_slb(
	const arsi_task_config_t *p_arsi_task_config,
	const arsi_lib_config_t  *p_arsi_lib_config,
	const data_buf_t         *p_param_buf,
	data_buf_t               *p_working_buf,
	...
	const debug_log_fp_t      debug_log_fp)
{
	...
	*pp_handler = p_working_buf->p_buffer;
	...
	/* slb */
	lib_handler->slb_buf.p_buffer = p_arsi_slb_buf->p_buffer;
	lib_handler->slb_buf.data_size = p_arsi_slb_buf->memory_size;
	lib_handler->slb_buf.memory_size = p_arsi_slb_buf->memory_size;
...
void demo_arsi_assign_lib_fp(AurisysLibInterface *lib)
{
	...
	lib->arsi_get_arsi_version = demo_arsi_get_arsi_version;
	lib->arsi_is_support_slb = demo_arsi_is_support_slb;
	lib->arsi_query_working_buf_size_with_slb =
		demo_arsi_query_working_buf_size_with_slb;
	lib->arsi_create_handler_with_slb = demo_arsi_create_handler_with_slb;
	lib->arsi_query_sync_buf = demo_arsi_query_sync_buf;
}
/* <-- For static link, like libXXX.a in FreeRTOS */
