...
#if !defined(CFG_INTERNAL_TIMER_SUPPORT) && !defined(CFG_CSR_XGPT_SUPPORT)
static portTickType sspm_systick = 0;
static unsigned int sspm_irq_val = 0;
...
void timer_irq_ack(unsigned int timer)
{
    ...
    if (timer == TIMER_ID0) {
        sspm_systick = xTaskGetTickCountFromISR();
        sspm_irq_val = DRV_Reg32(TIMER_IRQ_ACK(timer));
    }
