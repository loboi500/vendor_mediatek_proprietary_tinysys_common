    ...
    def parse_sections(self):
        ...
        for rawLine in self.f:
            ...
            if self.outOffset < length and rawLine[self.outOffset] != ' ':
                ...
                if inData.find('../prebuilts/') != -1:
                    inData = '/mtkeda/' + inData.replace('../', '')
                elif inData.find('../private/') != -1:
                    inData = 'private/scp/' + inData.replace('../private/', '')
                symbolData = ''
...
def parse_path(path, prefix):
    ...
    # add platform prefix path for library link
    elif path.find('.a') != -1:
        return prefix + '/' + '/'.join(path.split('/')[0:])
    else:
        return '/'.join(path.split('_intermediates/')[-1].split('/')[1:])
...
def query_feature(features, path):
    ...
    if path.find('heap') != -1:
        ...
        for k in sorted(features.iterkeys(), reverse=True, cmp=lambda k1, k2: cmp(len(k1), len(k2))):
            if path.find(k) != -1:
...
def main():
    ...
    if projType == 'SSPM':
        ...
        settings = ['TinySys-COMMON', 'TinySys-GPUEB']
    elif projType == 'VCP':
        settings = ['TinySys-COMMON', 'TinySys-VCP']
    ...
    for s in sectionS:
        if len(s) > sectionS_MaxLEN:
            sectionS_MaxLEN = len(s)
        for f in featureS:
            if len(f) > featureS_MaxLEN:
                featureS_MaxLEN = len(f)
            result1[(f, s)] = 0
            result3[f] = ""
        for f in subFeatureS:
            if len(f) > subFeatureS_MaxLEN:
                subFeatureS_MaxLEN = len(f)
            result2[(f, s)] = 0
            result3[f] = ""
    ...
    if dump: print("%*s " % (max(featureS_MaxLEN, subFeatureS_MaxLEN), ''), end='')
    if excel: index1x = 1; index1y = 1; chartMin = index1y + 1
    for s in sorted(sectionS):
        if dump: print("%*s " % (max(len(s), 7), s), end='')
        if excel:
            index1x += 1
            ws1.cell(row=index1y, column=index1x, value=s)
    if dump: print("%7s" % "Sum")
    if excel:
        index1x += 1
        ws1.cell(row=index1y, column=index1x, value="Sum")
    for f in sorted(featureS):
        if dump: print("%*s " % (max(featureS_MaxLEN, subFeatureS_MaxLEN), f), end='')
        if excel:
            index1y += 1
            index1x = 1
        ...
        for s in sorted(sectionS):
            v = result1.get((f, s), 0)
            result1[(f, 'sum')] += v
            if dump: print("%*d " % (max(len(s), 7), v), end='')
            if excel:
                index1x += 1
                ws1.cell(row=index1y, column=index1x, value=v)
        if dump: print("%7d" % result1[(f, 'sum')])
        if excel:
            index1x += 1
    ...
    for f in sorted(subFeatureS):
        ...
        for s in sorted(sectionS):
            ...
            if excel:
                index1x += 1
                ws1.cell(row=index1y, column=index1x, value=v)
        if dump: print("%7d" % result2[(f, 'sum')])
        if excel:
            index1x += 1
