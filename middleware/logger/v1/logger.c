...
#define PLATSEC
#endif
#ifndef LOGGER_OVERWRITE
#define LOGGER_OVERWRITE 0
#endif
...
volatile int logger_data_move_flag PLATSEC;	     /* logger data move flag,
						      ...
						      * 1: data moving */
#endif
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
struct logger_cs_timestamp_s {
	volatile uint64_t enter_cs;
	volatile uint64_t cache_op1;
	volatile uint64_t cache_op2;
	volatile uint64_t cache_op3;
	volatile uint64_t cache_op4;
	volatile uint64_t cache_op5;
	volatile uint64_t dma_start;
	volatile uint64_t dma_end;
	volatile uint64_t exit_cs;
};
struct logger_dma_argu_s {
	volatile unsigned char *logger_buf;
	volatile uint32_t logger_r_pos;
	volatile uint32_t dst_buf;
	volatile uint32_t dst_w_pos;
	volatile uint32_t src_len;
	volatile uint32_t dst_len;
};
struct logger_cs_timestamp_s logger_cs_timestamp;
struct logger_dma_argu_s logger_dma_argu;
#endif
...
void logger_update(const char *str, unsigned int src_len)
{
...
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG 
	logger_cs_timestamp.enter_cs = read_xgpt_stamp_ns();
#endif
	...
	logger_dcache_invalidate(&logger_r_pos, sizeof(unsigned int));
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
	logger_cs_timestamp.cache_op1 = read_xgpt_stamp_ns();
#endif
	if (logger_w_pos >= logger_r_pos)
...
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
	logger_cs_timestamp.cache_op2 = read_xgpt_stamp_ns();
#endif
	...
	if (dst_w_pos >= logger_dram_ctrl.buff_size
		...
		logger_dcache_flush(&logger_dram_inited, sizeof(unsigned int));
		printf("%s:err r=%u w=%u l=%u\n", __FUNCTION__, dst_r_pos,
			 dst_w_pos, logger_dram_ctrl.buff_size);
		goto exit;
	}
...
#endif
	{
		 ...
		 */
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
		logger_cs_timestamp.dma_start = read_xgpt_stamp_ns();
		logger_dma_argu.logger_buf = logger_buf;
		logger_dma_argu.logger_r_pos = logger_r_pos;
		logger_dma_argu.dst_buf = dst_buf;
		logger_dma_argu.dst_w_pos = dst_w_pos;
		logger_dma_argu.src_len = src_len;
		logger_dma_argu.dst_len = dst_len;
#endif
		len =
		    logger_update_dma((unsigned int) &logger_buf[0],
						      ...
						      src_len, dst_len);
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
		logger_cs_timestamp.dma_end = read_xgpt_stamp_ns();
#endif
	...
	logger_dcache_flush(&logger_r_pos, sizeof(unsigned int));
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
	logger_cs_timestamp.cache_op3 = read_xgpt_stamp_ns();
#endif
#endif
...
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
	logger_cs_timestamp.cache_op4 = read_xgpt_stamp_ns();
#endif

#if LOGGER_OVERWRITE
	if (overwrite) {
		ctrl->r_pos = dst_r_pos;
		logger_dcache_flush(&ctrl->r_pos, sizeof(unsigned int));
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
		logger_cs_timestamp.cache_op5 = read_xgpt_stamp_ns();
#endif
	}
#endif
      exit:
#ifdef CFG_LOGGER_CS_TIMEOUT_DEBUG
	logger_cs_timestamp.exit_cs = read_xgpt_stamp_ns();
#endif
				  ...
				  unsigned int limit)
{
	...
	if (!logger_dram_inited) {
		...
		logger_dcache_flush(&logger_dram_ctrl.enable, sizeof(unsigned int));
		logger_dcache_flush(&logger_dram_inited, sizeof(unsigned int));
