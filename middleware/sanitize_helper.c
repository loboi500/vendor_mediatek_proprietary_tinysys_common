...
#define SANI_SHOW(fmt, ...)	PRINTF_E(fmt, ##__VA_ARGS__)
#define SANI_ABORT()		configASSERT(0)

struct type_desc {
#define UB_SIGNED(type)		((type)->info&1)
#define UB_BITS(type)		(1U << ((type)->info >> 1))
#define UB_VAL(type, val)	(sizeof(long) * 8 >= UB_BITS(type) ? \
...
void __ubsan_handle_builtin_unreachable(struct src_loc *loc);
void __ubsan_handle_load_invalid_value(void *data, void *val);

static int ub_skip(struct src_loc *loc)
{
...
static void ub_val2str(char *str, unsigned size, struct type_desc *type, void *val)
{
	...
	if (UB_SIGNED(type)) {
		...
		x = ll_x = (extra_bits >= 0 ?
			((long)((unsigned long)val << extra_bits)) >> extra_bits : *(long long *)val);
		if (ll_x < 0) {
			x = -ll_x;
			*str++ = '-';
			size--;
...
static void ub_handle_type_mismatch(struct src_loc *loc, struct type_desc *type
	, unsigned long align, unsigned kind, void *ptr)
{
...
void __ubsan_handle_out_of_bounds(void *data, void *index)
{
	struct {
		struct src_loc loc;
		struct type_desc *type, *idx_type;
	} *inf = data;
	char str[24];
