...
{                                                                       \
    List_t *pxStateList;                                                \
                                                                        \
    pxStateList = (List_t *)listLIST_ITEM_CONTAINER(&(pxCurrentTCB[uxPortGetHartId()]->xStateListItem)); \
                                                                        \
    if ((pxStateList == pxDelayedTaskList[uxPortGetHartId()]) || (pxStateList == pxOverflowDelayedTaskList[uxPortGetHartId()])) \
    {                                                                   \
        event_task.type = EV_TASK_SWTCH_2;                              \
    }                                                                   \
    else if (pxStateList == &xSuspendedTaskList[uxPortGetHartId()])               \
    {                                                                   \
        if (listLIST_ITEM_CONTAINER(&(pxCurrentTCB[uxPortGetHartId()]->xEventListItem)) == NULL) \
        {                                                               \
    ...
    else if (pxStateList == &xTasksWaitingTermination[uxPortGetHartId()])         \
    {                                                                   \
    ...
    pbfr_task_switch(pxCurrentTCB[uxPortGetHartId()]->uxTCBNumber);                        \
}
...
{                                                                       \
    event_task.taskno = pxCurrentTCB[uxPortGetHartId()]->uxTCBNumber;                      \
    pbfr_rec_add_event((struct ev_rec_t*)(&event_task));                \
    freertos_current_task_no = pxCurrentTCB[uxPortGetHartId()]->uxTCBNumber;               \
